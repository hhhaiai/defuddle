...
<body>
  ...
  <script>
    ...
    function render() {
      ...
      try {
        preview.innerHTML = md.render(stripFrontmatter(src));
      } catch (e) {
        preview.innerHTML = '<p style="color:#b91c1c;">Render error: ' + escapeHtml(e.message || String(e)) + '</p>';
        return;
      }
    ...
    async function uploadImageToBaidu(imageUrl) {
      for (let attempt = 0; attempt <= retryDelaysMs.length; attempt++) {
        try {
          // Call the Worker API endpoint instead of Baidu directly. Browser direct
          // uploads are blocked by CORS, while the Worker can fetch the image and
          // send the Python-compatible Baidu payload server-side.
          ...
          console.warn('Image upload failed:', imageUrl, e);
    ...
    async function uploadAllImages() {
      ...
      const images = Array.from(imgTags).filter(img => {
        try {
          const host = new URL(img.src).hostname;
          return !host.includes('baidu.com') && !host.includes('bcebos.com');
        } catch {
          return false;
        }
      });
      ...
      for (const img of images) {
        ...
        uploadStatus.textContent = \`上传图片 \${uploaded + 1}/\${images.length}...\`;
